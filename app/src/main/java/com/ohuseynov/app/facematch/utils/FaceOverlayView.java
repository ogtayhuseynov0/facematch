...
public class FaceOverlayView extends View {
    ...
    public void setBitmap(Bitmap bitmap) {
        ...
        if (!detector.isOperational()) {
            //Handle contingency
            Log.e("LOg", "Detector not operational```````````````````````");
            infov.setText("Face detection is not operational on your device!!\n" +
                    "GSM started to download library. It will be available as soon as possible");
            infov.setTextColor(Color.RED);
        } else {
    ...
    private void logFaceData() {
        ...
        float eulerY;
        float eulerZ;
        String info="Face Count: "+ mFaces.size()+"\n";
        if (mFaces.size()>0){
            ...
            info+="Smiling: " + smilingProbability+"\n"+
                    "Left eye open: " + leftEyeOpenProbability+"\n"+
                    "Right eye open: " + rightEyeOpenProbability;
            if (mFaces.size()>1){
                infov.setText(Html.fromHtml("<b style=\" color: red\">Profile picture can only have one Face </b> <br/>\n"+
                        "Face Count: "+ mFaces.size()+"\n"));
            }else {
                infov.setText(info);
            }
            ...
            infov.setText(Html.fromHtml("<b style=\" color: red\">Profile picture must have a face </b> <br/>\n"+
                    "Face Count: "+ mFaces.size()+"\n"));
        }

    }

    public TextView getInfov() {
        return infov;
    }

    public void setInfov(TextView infov) {
        this.infov = infov;
    }
}